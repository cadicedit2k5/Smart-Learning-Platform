...
public class ApiLearningProgressController {
    ...
    public ResponseEntity<ApiResponse<TopicLearningProgressResponse>>
    startTopic(
    ...
    @PreAuthorize(Authorities.COURSE_READ)
    public ResponseEntity<ApiResponse<CourseLearningProgressResponse>>
    getMyProgress(
            @PathVariable UUID courseId,
            @AuthenticationPrincipal Jwt jwt
    ) {
        return ApiResponses.ok(
                progressService.getMyCourseProgress(
                        courseId,
                        JwtUtils.getUserId(jwt)
                )
        );
