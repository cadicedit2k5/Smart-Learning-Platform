...
const descriptionPreview = (description: string | null) => storedRichTextToPlainText(description)
</script>
...
<template>
  <section class="overflow-hidden rounded-card border border-app-border bg-app-surface shadow-card">
    <header class="flex items-center justify-between border-b border-app-border px-5 py-4">
      <div>
        <h2 class="font-heading font-bold text-app-text">Danh sách môn học</h2>
        <p class="mt-1 text-xs text-app-text-muted">{{ total }} môn học</p>
      </div>
    </header>

    <div v-if="loading" class="space-y-3 p-5">
      <div v-for="index in 5" :key="index" class="h-16 animate-pulse rounded-control bg-app-surface-muted" />
    </div>

    <div v-else-if="courses.length === 0" class="p-5">
      <BaseEmptyState
      ...
      <table class="w-full min-w-[950px] text-left text-sm">
        <thead class="bg-app-surface-muted text-xs uppercase tracking-wide text-app-text-muted">
          <tr>
            ...
            <th class="px-5 py-3">Cấp độ</th>
            <th class="px-5 py-3">Trạng thái</th>
            <th class="px-5 py-3">Phạm vi</th>
        ...
        <tbody class="divide-y divide-app-border">
          ...
          >
            <td class="max-w-[350px] px-5 py-4">
              <p class="truncate font-semibold text-app-text">{{ course.title }}</p>

              <p
                v-if="descriptionPreview(course.description)"
            ...
            <td class="px-5 py-4 text-app-text-muted">{{ course.level || '—' }}</td>
            ...
            <td class="px-5 py-4">{{ visibilityLabel(course.visibility) }}</td>
