...
import {
  ...
  requestToJoinCourse,
  type PublicCourseChapterOutline,
  ...
  course.value?.chapters.reduce(
    (courseTotal, chapter) =>
      ...
      chapter.topics.reduce(
        (chapterTotal, topic) =>
          chapterTotal + (topic.estimatedMinutes ?? 0),
        0,
      ),
    0,
  ) ?? 0,
...
const estimatedDuration = computed(() =>
  formatDuration(totalEstimatedMinutes.value),
)

const chapterDuration = (chapter: PublicCourseChapterOutline) => {
  const minutes = chapter.topics.reduce(
  ...
  <section class="mx-auto max-w-[76rem] space-y-7 pb-10">
    ...
    <BaseAlert v-else-if="loadError">
      <div class="flex flex-wrap items-center justify-between gap-3">
        <span>{{ loadError }}</span>
        <BaseButton variant="secondary" @click="loadCourse">
          Thử lại
        </BaseButton>
      </div>
    </BaseAlert>
    ...
    <template v-else-if="course">
      <header
        class="relative overflow-hidden rounded-[1.75rem] border border-app-border bg-gradient-to-br from-white via-slate-50 to-secondary-soft/60 shadow-card"
      >
        <div
        ...
        <div
          class="pointer-events-none absolute -bottom-28 left-1/4 h-64 w-64 rounded-full bg-primary/5 blur-3xl"
        />

        <div
          class="relative grid items-center gap-8 p-6 sm:p-8 lg:grid-cols-[minmax(0,1fr)_25rem] lg:p-10"
        >
          <div class="max-w-3xl">
            <div class="flex flex-wrap items-center gap-2">
              <span
                class="rounded-pill border border-secondary/15 bg-secondary-soft px-3 py-1.5 text-xs font-semibold text-secondary"
              >
                Khóa học công khai
              </span>
              ...
              <span
                v-if="course.level"
                class="rounded-pill border border-app-border bg-white/80 px-3 py-1.5 text-xs font-semibold text-app-text-muted"
              >
                {{ course.level }}
              </span>
            </div>
            ...
            <h1
              class="mt-6 max-w-3xl font-heading text-3xl font-bold leading-tight tracking-tight text-app-text sm:text-4xl lg:text-5xl"
            >
              {{ course.title }}
            </h1>

            <p
              class="mt-5 max-w-2xl text-base leading-7 text-app-text-muted sm:text-lg"
            >
              {{ descriptionPreview }}
            </p>
            ...
            <div class="mt-8 flex flex-wrap gap-x-6 gap-y-4">
              <div class="flex items-center gap-2 text-sm text-app-text-muted">
                ...
                <span>
                  <strong class="font-semibold text-app-text">
                    {{ chapterCount }}
                  </strong>
                  chương
                </span>
          ...
          <div
            class="overflow-hidden rounded-[1.25rem] border border-app-border bg-white p-2 shadow-overlay"
          >
            <CourseCover
      ...
      <div class="grid items-start gap-7 lg:grid-cols-[minmax(0,1fr)_20rem]">
        <main class="min-w-0 space-y-8">
          <section
            class="rounded-[1.25rem] border border-app-border bg-app-surface p-6 shadow-card sm:p-7"
          >
            <div class="flex items-start gap-4">
              <div
                class="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-secondary-soft text-secondary"
              >
          ...
          <section>
            ...
            >
              <div>
                ...
                <p class="mt-2 text-sm text-app-text-muted">
                  Xem cấu trúc khóa học trước khi gửi yêu cầu tham gia.
                </p>
              </div>

              <p
                v-if="chapterCount"
                class="shrink-0 text-sm font-medium text-app-text-muted"
              >
            ...
            <div v-else class="space-y-3">
              ...
              >
                <summary
                  class="flex cursor-pointer list-none items-center gap-4 px-5 py-5 sm:px-6"
                >
                  <div
                    class="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-app-surface-muted font-heading text-sm font-bold text-app-text"
                  >
                  ...
                  <div class="min-w-0 flex-1">
                    <h3
                      class="font-heading text-base font-bold text-app-text sm:text-lg"
                    >
                      {{ chapter.title }}
                    </h3>

                    <div
                      class="mt-1.5 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-app-text-muted"
                    >
                      <span>{{ chapter.topics.length }} bài học</span>

                      <span
                        v-if="chapterDuration(chapter) !== 'Chưa xác định'"
                  ...
                  <div
                    class="flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-app-text-muted transition group-hover:bg-app-surface-muted"
                  >
                    <ChevronDown
                      :size="19"
                ...
                <div
                  class="border-t border-app-border bg-slate-50/60 px-5 py-5 sm:px-6"
                >
                  <div
                  ...
                  >
                    <div v-if="chapter.description">
                      <p
                        class="text-xs font-bold uppercase tracking-wide text-app-text-muted"
                      >
                        Giới thiệu chương
                      </p>

                      <p
                        class="mt-2 whitespace-pre-wrap text-sm leading-6 text-app-text"
                      >
                        {{ chapter.description }}
                      </p>
                    </div>
                  ...
                  <div
                    v-if="chapter.topics.length"
                    class="overflow-hidden rounded-xl border border-app-border bg-app-surface"
                  >
                    <div
                    ...
                    >
                      <div
                        class="mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-app-surface-muted text-app-text-muted"
                      >
                        <LockKeyhole :size="15" />
                      </div>
                      ...
                      <div class="min-w-0 flex-1">
                        ...
                        >
                          <div class="min-w-0">
                            ...
                            <h4
                              class="mt-0.5 font-semibold leading-6 text-app-text"
                            >
                              {{ topic.title }}
                            </h4>
                          </div>
                          ...
                          <span
                            ...
                            class="flex shrink-0 items-center gap-1.5 text-xs text-app-text-muted"
                          >
                            <Clock3 :size="14" />
                        ...
                        <p
                          ...
                          class="mt-1.5 line-clamp-2 text-sm leading-6 text-app-text-muted"
                        >
                          {{ topic.description }}
                        </p>
                      </div>
                  ...
                  <p
                    v-else
                    class="rounded-xl border border-dashed border-app-border bg-app-surface px-4 py-5 text-center text-sm text-app-text-muted"
                  >
        ...
        <aside class="order-first lg:order-last lg:sticky lg:top-6">
          <section
            class="overflow-hidden rounded-[1.25rem] border border-app-border bg-app-surface shadow-overlay"
          >
            <div class="p-5">

              <h2 class="mt-2 font-heading text-xl font-bold text-app-text">
                Bắt đầu học cùng lớp
              </h2>
            ...
            <div class="border-t border-app-border px-5 py-5">
              ...
              <div class="mt-4 space-y-4">
                <div class="flex items-center gap-3">
                  <div
                    class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-app-surface-muted text-app-text-muted"
                  >
            ...
            >
              <div class="flex gap-3">
                <LockKeyhole
                  :size="17"
                  class="mt-0.5 shrink-0 text-app-text-muted"
                />
