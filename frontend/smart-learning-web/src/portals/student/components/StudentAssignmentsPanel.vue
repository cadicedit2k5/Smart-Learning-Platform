...
import { BaseAlert, BaseButton, BaseModal, ConfirmDialog } from '@/shared/components'
...
type StudentAssignmentStatus = 'GRADED' | 'SUBMITTED' | 'CLOSED' | 'NOT_SUBMITTED'
...
const submissionByAssignment = computed(() => {
  ...
  for (const submission of submissions.value) map.set(submission.assignmentId, submission)
  return map
})
...
const canSubmit = computed(() => selectedAssignment.value?.status === 'PUBLISHED')
...
const currentSubmissionContent = computed(() => parseStoredRichText(currentSubmission.value?.content))
const hasCurrentSubmissionContent = computed(() => hasRichTextContent(currentSubmissionContent.value))

const preview = (value: string | null) => storedRichTextToPlainText(value)

const assignmentStatus = (assignment: Assignment): StudentAssignmentStatus => {
...
const assignmentStatusClass = (assignment: Assignment) => {
  const status = assignmentStatus(assignment)

  if (status === 'GRADED') return 'bg-ai-soft text-ai'
  if (status === 'SUBMITTED') return 'bg-secondary-soft text-secondary'
  if (status === 'CLOSED') return 'bg-danger-soft text-danger'
...
const load = async () => {
  ...
  try {
    ...
    message.value = handleApiError(error, 'Không thể tải danh sách bài tập.').message
  } finally {
    loading.value = false
  }
}
...
const resetSubmissionForm = () => {
  submissionContent.value = createEmptyRichText()
  file.value = null
...
const requireOpenAssignment = () => {
  if (canSubmit.value) return true

  message.value = 'Bài tập đã đóng và không còn nhận bài nộp.'
  return false
}

const openCreateSubmission = () => {
  if (!selectedAssignment.value || !requireOpenAssignment()) return
...
const openEditSubmission = () => {
  if (!currentSubmission.value || !requireOpenAssignment()) return

  submissionContent.value = parseStoredRichText(currentSubmission.value.content)
  file.value = null
  fileInputKey.value += 1
  submissionMode.value = 'edit'
  submissionModalOpen.value = true
...
const submit = async () => {
  ...
  const serializedContent = serializeRichText(submissionContent.value)

  if (!serializedContent && !file.value && !currentSubmission.value?.attachmentUrl) {
  ...
  try {
    ...
    const index = submissions.value.findIndex(item => item.assignmentId === result.assignmentId)

    if (index >= 0) submissions.value[index] = result
    else submissions.value.push(result)
...
const handleDeleteSubmission = async () => {
  if (!selectedAssignment.value || !currentSubmission.value || !requireOpenAssignment()) return
  ...
  try {
    ...
    message.value = handleApiError(error, 'Không thể xóa bài đã nộp.').message
  } finally {
...
<template>
  <section class="space-y-6">
    ...
    <template v-if="!selectedAssignment">
      <header>
        <p class="text-xs font-bold uppercase tracking-[0.14em] text-secondary">Course work</p>
        <h2 class="mt-1 font-heading text-2xl font-bold text-app-text">Bài tập</h2>
        <p class="mt-1 text-sm text-app-text-muted">Theo dõi yêu cầu, hạn nộp và kết quả bài tập.</p>
      </header>
      ...
      >
        ...
        <p class="mt-1 text-sm text-app-text-muted">Giảng viên chưa giao bài tập cho khóa học.</p>
      </div>
        ...
        >
          ...
          <div class="min-w-0">
            <div class="flex flex-wrap items-center gap-2">
              <h3 class="font-heading font-bold text-app-text">{{ assignment.title }}</h3>

              <span class="rounded-pill px-2.5 py-1 text-xs font-semibold" :class="assignmentStatusClass(assignment)">
      ...
      <section class="overflow-hidden rounded-[1.3rem] border border-app-border bg-app-surface shadow-card">
        <header class="border-b border-app-border bg-gradient-to-br from-white to-secondary-soft/30 px-6 py-6 sm:px-7">
          <div class="flex flex-wrap items-center gap-2">
            <span
              ...
              :class="selectedAssignment.status === 'PUBLISHED'
                ? 'bg-ai-soft text-ai'
                : 'bg-danger-soft text-danger'"
            >
              {{ selectedAssignment.status === 'PUBLISHED' ? 'Đang mở' : 'Đã đóng' }}
            </span>

            <span
              v-if="assignmentStatus(selectedAssignment) === 'GRADED'"
        ...
        <div class="grid lg:grid-cols-[minmax(0,1fr)_19rem]">
          <article class="p-6 sm:p-7">
            <h3 class="font-heading text-lg font-bold text-app-text">Nội dung bài tập</h3>

            <div class="mt-5">
              <RichTextViewer v-if="hasSelectedDescription" :content="selectedDescription" />
              <p v-else class="text-sm text-app-text-muted">Bài tập không có nội dung hướng dẫn.</p>
            </div>
          </article>

          <aside class="border-t border-app-border bg-app-surface-muted/40 p-6 lg:border-l lg:border-t-0">
            <template v-if="!currentSubmission">
              <FileText :size="21" class="text-app-text-muted" />

              <h3 class="mt-4 font-heading font-bold text-app-text">Bài làm của bạn</h3>

              <p class="mt-2 text-sm leading-6 text-app-text-muted">
                {{ canSubmit ? 'Bạn chưa nộp bài tập này.' : 'Bài tập đã đóng và không còn nhận bài.' }}
              </p>

              <BaseButton v-if="canSubmit" class="mt-5" block @click="openCreateSubmission">
                <template #leading><Upload :size="16" /></template>
                Làm bài
              </BaseButton>
            </template>
            ...
            <template v-else>
              <ClipboardCheck
                :size="21"
                :class="currentSubmission.status === 'GRADED' ? 'text-ai' : 'text-secondary'"
              />

              <h3 class="mt-4 font-heading font-bold text-app-text">
                {{ currentSubmission.status === 'GRADED' ? 'Đã chấm điểm' : 'Bài đã nộp' }}
              </h3>

              <p class="mt-2 text-sm text-app-text-muted">
                Nộp {{ formatDateTime(currentSubmission.submittedAt) }}
              </p>
              ...
              >
                {{ currentSubmission.score }} / {{ selectedAssignment.maxScore }}
              </p>

              <BaseButton class="mt-5" block variant="secondary" @click="openViewSubmission">
    ...
    <BaseModal
      ...
      :title="submissionMode === 'view'
        ? 'Bài đã nộp'
        : currentSubmission
          ? 'Chỉnh sửa bài nộp'
          : 'Nộp bài tập'"
      :description="submissionMode === 'view'
    ...
    >
      <div v-if="currentSubmission && submissionMode === 'view'" class="space-y-6">
        <div class="flex flex-wrap items-center gap-2">
          <span
            class="rounded-pill px-2.5 py-1 text-xs font-semibold"
            :class="currentSubmission.status === 'GRADED'
              ? 'bg-ai-soft text-ai'
              : 'bg-secondary-soft text-secondary'"
          >
            {{ currentSubmission.status === 'GRADED' ? 'Đã chấm' : 'Chờ chấm' }}
          </span>
        ...
        <section>
          ...
          <div class="mt-3 rounded-[1rem] border border-app-border bg-app-surface p-5">
            <RichTextViewer
              v-if="hasCurrentSubmissionContent"
              :content="currentSubmissionContent"
            />
        ...
        >
          <div class="flex h-10 w-10 items-center justify-center rounded-xl bg-secondary-soft text-secondary">
            <Paperclip :size="17" />
          </div>

          <div class="min-w-0 flex-1">
            <p class="truncate text-sm font-semibold text-app-text">
              {{ currentSubmission.originalFileName || 'Tệp đính kèm' }}
            </p>
            <p class="mt-0.5 text-xs text-app-text-muted">Mở tệp đính kèm</p>
          </div>
        </a>
        ...
        >
          ...
          >
            <p class="text-xs font-semibold text-ai">Nhận xét của giảng viên</p>
            <p class="mt-2 whitespace-pre-wrap text-sm leading-6 text-app-text-muted">
        ...
        <div class="flex justify-end gap-3 border-t border-app-border pt-5">
          <template v-if="currentSubmission.status !== 'GRADED' && canSubmit">
            <button
              type="button"
            ...
            <BaseButton variant="secondary" @click="openEditSubmission">
              Chỉnh sửa
            </BaseButton>
          </template>

          <BaseButton @click="closeSubmissionModal">Đóng</BaseButton>
        </div>
      </div>
      ...
      <form v-else class="space-y-6" @submit.prevent="submit">
        <div>
          <div class="mb-2 flex items-center justify-between gap-3">
            <label class="text-sm font-semibold text-app-text">Nội dung bài làm</label>
            <span class="text-xs text-app-text-muted">Hỗ trợ định dạng văn bản</span>
          </div>

          <RichTextEditor v-model="submissionContent" :disabled="submitting" />
        </div>

        <div>
          <label class="text-sm font-semibold text-app-text">Tệp đính kèm</label>

          <div class="mt-2 rounded-[1rem] border border-dashed border-app-border bg-app-surface-muted/30 p-4">
            <input
              :key="fileInputKey"
              type="file"
              :disabled="submitting"
            ...
            <p
              v-if="currentSubmission?.originalFileName"
            ...
            <p class="mt-2 text-xs text-app-text-muted">
              Có thể nộp nội dung, tệp đính kèm hoặc cả hai.
            </p>
          </div>
        </div>
        ...
        <div class="flex justify-end gap-3 border-t border-app-border pt-5">
          ...
          <BaseButton type="submit" :loading="submitting">
            <template #leading><Upload :size="16" /></template>
            {{ currentSubmission ? 'Cập nhật bài nộp' : 'Nộp bài' }}
          </BaseButton>
        </div>
      </form>
    </BaseModal>
