...
const conversationsPage = ref<PaginatedData<AiConversation>>(emptyPage())
const messagesPage = ref<PaginatedData<AiMessage>>(emptyPage())
...
const conversationTotalPages = computed(() =>
  Math.max(1, conversationsPage.value.pageable.totalPages),
)
const messageTotalPages = computed(() => Math.max(1, messagesPage.value.pageable.totalPages))
const selectedConversation = computed(() =>
  conversationsPage.value.content.find((item) => item.id === selectedConversationId.value),
)
...
const goToConversationPage = (nextPage: number) => {
  if (
    nextPage < 1 ||
    nextPage > conversationTotalPages.value ||
    nextPage === conversationPage.value
  )
    return
...
const goToMessagePage = (nextPage: number) => {
  if (nextPage < 1 || nextPage > messageTotalPages.value || nextPage === messagePage.value) return
...
const submitPrompt = async () => {
  ...
  if (!content) {
    promptError.value = 'Vui lòng nhập câu hỏi.'
    return
  }
...
const handlePromptKeydown = (event: KeyboardEvent) => {
  if (event.key === 'Enter' && !event.shiftKey) {
    ...
    if (!sending.value) void submitPrompt()
  }
}
  ...
  <section class="space-y-5">
    <header>
      <div class="flex items-center gap-2 text-ai">
        <Sparkles :size="19" /><span class="text-sm font-semibold">AI tutor theo khóa học</span>
      </div>
    ...
    >
      ...
      >
        <div class="border-b border-app-border p-4">
          <BaseButton variant="ai" block @click="startNewConversation"
            ><template #leading><Plus :size="18" /></template>Hội thoại mới</BaseButton
          >
        </div>

        <BaseAlert v-if="conversationError" class="m-3">{{ conversationError }}</BaseAlert>
      ...
      <div class="flex min-h-[620px] min-w-0 flex-col">
        <header class="border-b border-app-border px-5 py-4">
          ...
          <p class="mt-1 text-xs text-app-text-muted">Enter để gửi, Shift + Enter để xuống dòng.</p>
        </header>
        ...
        <div ref="messageList" class="min-h-0 flex-1 space-y-5 overflow-y-auto p-4 sm:p-6">
          ...
          >
            ...
            <p class="mt-2 max-w-md text-sm leading-6 text-app-text-muted">
              Hỏi về nội dung hoặc tài liệu hiện có trong khóa học.
            </p>
          </div>
          <template v-else>
            <article
            ...
            >
              <span
                ...
                "
              >
                <UserRound v-if="message.role === 'USER'" :size="18" /><Bot v-else :size="18" />
              </span>
        ...
        <form class="border-t border-app-border p-4" @submit.prevent="submitPrompt">
          ...
          >
            ...
            <button
              ...
              class="mb-1 flex h-10 w-10 shrink-0 items-center justify-center rounded-control bg-ai text-white disabled:opacity-50"
